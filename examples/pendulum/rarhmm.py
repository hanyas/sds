...
if __name__ == "__main__":
    ...
    _, _sample_obs = rarhmm.sample([act[_idx]], horizon=[200])
    ...
    # error
    mse, norm_mse = rarhmm.kstep_mse(obs, act, horizon=50,
    ...
    for _obs, _act in zip(obs, act):
        hist_obs.append(_obs[:T + 1, :])
        hist_act.append(_act[:T + 1, :])
        nxt_act.append(_act[T: T + 1 + H, :])

    hr = [H for _ in hist_obs]
    _, obs_hat = rarhmm.forcast(hist_obs=hist_obs, hist_act=hist_act,
