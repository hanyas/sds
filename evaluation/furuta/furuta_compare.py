...
def fit_rarhmm(obs, act, nb_states):
    ...
    obs_prior = {'mu0': 0., 'sigma0': 1e32, 'nu0': dm_obs + 2, 'psi0': 1.}
    ...
    rarhmm.em(obs=obs, act=act,
              nb_iter=350, prec=1e-2, verbose=True,
...
def parallel_rarhmm_fit(obs, act, nb_states, nb_jobs):
    ...
    return Parallel(8, verbose=10, backend='loky')(map(delayed(fit_rarhmm_job), args))


def parallel_rarhmm_test(models, obs, act, horizon):
    args = [(model, obs, act, horizon) for model in models]
    results = Parallel(8, verbose=10, backend='loky')(map(delayed(test_rarhmm_job), args))
...
def fit_fnn(obs, act, nb_epochs=5000):
    ...
    fnn = DynamicNNRegressor([input.shape[-1], 18, 18, target.shape[-1]])
...
def fit_rnn(obs, act, nb_epochs=5000):
    ...
    rnn = DynamicRNNRegressor(input_size, target_size, hidden_size=18, nb_layers=2)
...
def fit_lstm(obs, act, nb_epochs=100):
    ...
    lstm = DynamicLSTMRegressor(input_size, target_size, [18, 18])
...
if __name__ == "__main__":
    ...
    save("rarhmm_furuta_mse.tex")
    # save("gp_furuta_mse.tex")
    # save("fnn_furuta_mse.tex")
    # save("rnn_furuta_mse.tex")
    # save("lstm_furuta_mse.tex")
    plt.close()
    ...
    save("rarhmm_furuta_evar.tex")
    # save("gp_furuta_evar.tex")
    # save("fnn_furuta_evar.tex")
    # save("rnn_furuta_evar.tex")
    # save("lstm_furuta_evar.tex")
    plt.close()
