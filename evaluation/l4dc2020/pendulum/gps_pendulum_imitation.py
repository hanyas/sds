...
if __name__ == "__main__":
    ...
    import pickle
    data = pickle.load(open("./data/gps_pendulum_polar.pkl", "rb"))
    ...
    for _obs, _act in zip(raw, act):
        _raw = np.vstack((normalize(_obs[:, 0]), _obs[:, 1])).T
        _trig = np.vstack((np.cos(_raw[:, 0]), np.sin(_raw[:, 0]), _raw[:, 1])).T
        obs.append(_trig)
    ...
    for roll in rollouts[:25]:
        ...
        ax.scatter(angle[::3], roll['x'][::3, 2], color='r', s=1.2)
    ax = beautify(ax)
    ...
    # save("gps_pendulum_imitation_phase.tex")
