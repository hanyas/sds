...
def parallel_arhmm_fit(obs, act, options, nb_jobs):
    ...
    def fit_arhmm_job(args):
        ...
        rarhmm.em(obs=obs, act=act, verbose=True,
                  nb_iter=options['nb_iter'], prec=1e-2,
...
def parallel_rarhmm_fit(obs, act, options, nb_jobs, gpu):
...
def parallel_rarhmm_test(models, obs, act, horizon, gpu):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Compare SOTA Models on Furuta')
    ...
    parser.add_argument('--nb_states', help='number of linear components', default=25, type=int)
    ...
    if args.model == 'arhmm':
        ...
        options = {'dm_obs': env.dm_obs, 'dm_act': env.dm_act,
                   'nb_states': args.nb_states, 'nb_iter': 250,
                   'obs_prior': obs_prior,
    ...
    if args.model == 'rarhmm':
        ...
        if args.env == 'cart':
            ...
            trans_kwargs = {'hidden_layer_sizes': (64,), 'nonlinearity': 'splus',
            ...
            trans_kwargs = {'hidden_layer_sizes': (64,), 'nonlinearity': 'relu',
                            'device': 'gpu' if args.gpu else 'cpu',
        ...
        options = {'dm_obs': env.dm_obs, 'dm_act': env.dm_act,
                   'nb_states': args.nb_states, 'nb_iter': _nb_iter,
        ...
        for h in hr:
            print("Horizon: ", h)
            mse, smse, evar = parallel_rarhmm_test(rarhmms, test_obs, test_act,
                                                   int(h), gpu=args.gpu)
    ...
    save(str(args.env) + "_furuta_" + str(model_string) + "_mse.tex")
    plt.close()
    ...
    save(str(args.env) + "_furuta_" + str(model_string) + "_smse.tex")
    plt.close()
    ...
    save(str(args.env) + "_furuta_" + str(model_string) + "_evar.tex")
    plt.close()
