...
if __name__ == "__main__":
    ...
    _ctl = SAC.load("./data/sac_cartpole_cart")
    sac_ctl = lambda x: _ctl.predict(x)[0]
    nb_rollouts, nb_steps = 50, 500
    obs, act = sample_env(env, nb_rollouts, nb_steps, sac_ctl, np.sqrt(1e-1))

    fig, axs = plt.subplots(nrows=1, ncols=5, figsize=(25, 6))
    fig.suptitle('Cartpole SAC Demonstrations')
    ...
    trans_type = 'neural'
    trans_prior = {'l2_penalty': 1e-32, 'alpha': 1, 'kappa': 10}
    ...
    for roll in rollouts[:25]:
        ...
        ax.scatter(angle[::3], roll['x'][::3, 4], color='r', s=1.5)
    ax = beautify(ax)
    ...
    # save("sac_cartpole_imitation_phase.tex")
