...
def fit_rarhmm(obs, act, nb_states):
    ...
    rarhmm.em(obs=obs, act=act,
              nb_iter=100, prec=1e-2, verbose=True,
...
def fit_fnn(obs, act, nb_epochs=5000):
    ...
    fnn = DynamicNNRegressor([input.shape[-1], 8, 8, target.shape[-1]])
...
def fit_rnn(obs, act, nb_epochs=5000):
    ...
    rnn = DynamicRNNRegressor(input_size, target_size, hidden_size=8, nb_layers=2)
...
def fit_lstm(obs, act, nb_epochs=100):
    ...
    lstm = DynamicLSTMRegressor(input_size, target_size, [8, 8])
...
if __name__ == "__main__":
    ...
    save("bouncing_" + str(args.model) + "_mse.tex")
    plt.close()
    ...
    save("bouncing_" + str(args.model) + "_evar.tex")
    plt.close()
